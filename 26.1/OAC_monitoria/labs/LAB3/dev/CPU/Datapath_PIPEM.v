...
// 3 fios para usar como quiser 					RegIDEX[274:272] <= wID_COrigPC;		  // 3
					RegIDEX[NTYPE+275-1:275] <= wID_InstrType; // 14
`ifdef RV32IMF
...
wire fsw;
	assign fsw=RegIDEX[83]; //wEX_InstrType[9];   Gambiarra PDC
	
wire [31:0] wEX_ForwardB2;
always @(*)
	case(fsw)	
		...
		if (wEXMEM_Flush)
			...
			if (!wEX_Stall) 
				begin
					...
					RegEXMEM[ 95: 64] <=  wEX_ForwardB2; //wEX_InstrType[ 9] ? wEX_FRead2 : wEX_ForwardB;		//32 **********************************************
